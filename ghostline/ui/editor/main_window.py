...
class MainWindow(QMainWindow):
    ...
    def __init__(
        ...
        # Register core commands before creating UI components that depend on them
        ...
        # Now create editor tabs after commands are registered
    ...
    def _run_current_file(self, file_path: str | None = None) -> None:
        ...
        print(f"Prepared command: {command}")  # Debug log
        ...
        print(f"Terminal: {terminal}, Terminal Dock: {terminal_dock}")  # Debug log
        
        if terminal is not None and terminal_dock is not None:
            try:
                print("Showing terminal dock...")  # Debug log
                self._show_and_raise_dock(terminal_dock, tool_id=None)
                print("Terminal dock shown")  # Debug log

                # If the terminal has a helper, use it, otherwise just write.
                if hasattr(terminal, "run_command"):
                    print("Using terminal.run_command")  # Debug log
                    terminal.run_command(command)
                else:
                    print("Falling back to direct process write")  # Debug log
                    # Minimal fallback: send the command directly to the shell.
                    terminal.output.appendPlainText(command)
                    terminal.process.write((command + "\n").encode())
                return
            except Exception as e:
                print(f"Error using terminal: {e}")  # Debug log
                # If anything goes wrong showing the dock, fall back to tasks
                self.task_manager.run_command("Run", command, cwd=str(path.parent))
                return

        print("Falling back to task manager")  # Debug log
